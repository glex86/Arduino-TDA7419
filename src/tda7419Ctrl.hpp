...
namespace TDA7419 {
	...
	public:
		...
		explicit TDA7419Ctrl(TDA7419 &device) : dev(device), treble(*this), bass(*this),
			...
			mixing(*this), spectrum(*this), speaker(*this) {}
		 ...
		 * @note Forwards to TDA7419::sendAllRegisters().
		 */
		bool sendAllRegisters() { return dev.sendAllRegisters(); }

        /**
		 * @brief Send only registers that have changed since last transmission.
		 * @return bool true on success.
		 * @note Forwards to TDA7419::sendChangedRegisters().
		 */
        bool sendChangedRegisters() { return dev.sendChangedRegisters(); }
	};
